 ...
 *
 * File name | Nazwa pliku: cms-users-messages-table.component.ts
...
/**
 * Komponent odpowiedzialny za renderowanie tabeli wiadomości wszystkich użytkowników.
 */

@Component({
    selector: 'app-cms-users-messages-table',
    templateUrl: './cms-users-messages-table.component.html',
    styleUrls: [],
    providers: [ CmsGetTablesConnectorService ],
})
export class CmsUsersMessagesTableComponent implements OnInit, OnDestroy {

    public _usersMessagesPagination?: CmsPaginationDataModel<CmsSingleUserMessageDataModel>;
    public _userMessagesSortBy: typeof BasicDataSortBy = BasicDataSortBy;

    public _deleteUsersMessages: Array<number> = new Array<number>();
        ...
        this._store.pipe(
            ...
            if (data.pageSize !== 1) {
                this._serviceGET
                    ...
                    .subscribe(filteredPagination => this._usersMessagesPagination = filteredPagination);
            }
        });
    };

    public emitDeleteArrayValues(deletedValues: Array<number>): void {
        this._deleteUsersMessages = deletedValues;
    };
