 ...
 *
 * File name | Nazwa pliku: dashboard-database-elements-plot.component.ts
...
/**
 * Komponent odpowiadającyh za renderowanie widoku wykresu kołowego ze wszystkimi ilościami danych znajdujących się
 * w bazie danych.
 */

@Component({
    selector: 'app-dashboard-database-elements-plot',
    templateUrl: './dashboard-database-elements-plot.component.html',
    styleUrls: [],
})
export class DashboardDatabaseElementsPlotComponent implements AfterViewInit {
    ...
    public ngAfterViewInit(): void {
        const allElements = document.querySelectorAll('.database-circles___svg') as NodeListOf<SVGAElement>;
        let prevSize: number = 0;
        [ ...allElements ].reverse().forEach((el, idx) => {
            const dataObject = this._dashboardDetailsData.dashboardElementsCount;
