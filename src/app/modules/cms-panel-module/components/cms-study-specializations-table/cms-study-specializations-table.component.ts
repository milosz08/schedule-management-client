 ...
 *
 * File name | Nazwa pliku: cms-study-specializations-table.component.ts
...
import { CmsPaginationDataModel, CmsSingleStudySpecDataModel } from '../../models/cms-pagination-data.model';
...
/**
 * Komponent odpowiedzialny za renderowanie widoku tabeli kierunków studiów (wraz z możliwością paginacji i
 * zaawansowanego sortowania rekordów).
 */

@Component({
    selector: 'app-cms-study-specializations-table',
    templateUrl: './cms-study-specializations-table.component.html',
    styleUrls: [],
    providers: [ CmsGetTablesConnectorService ],
})
export class CmsStudySpecializationsTableComponent implements OnInit, OnDestroy {

    public _studySpecsPagination?: CmsPaginationDataModel<CmsSingleStudySpecDataModel>;
    public _studySpecsSortBy: typeof BasicDataSortBy = BasicDataSortBy;

    public _deleteStudySpecs: Array<number> = new Array<number>();
    ...
    public ngOnInit(): void {
        ...
        this._store.pipe(
            ...
            if (data.pageSize !== 1) {
                this._serviceGET
                    .getAllStudySpecs(new PaginationNavSender(data))
                    .pipe(takeUntil(this._unsubscribe))
                    .subscribe(filteredPagination => {
                        this._studySpecsPagination = filteredPagination;
                    });
            }
        });
    };
    ...
    public emitDeleteArrayValues(deletedValues: Array<number>): void {
        this._deleteStudySpecs = deletedValues;
    };
